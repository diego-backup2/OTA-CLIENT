...
function selectOffer(self)
  if self.data.unlocked then return false end
    ...
    if widget then
      ...
      elseif type(offersCache[i].id) == "number" then
        ...
        if categoryId == CATEGORY_ITEM then
          ...
          if item:getItem():isStackable() then
            item:setItemCount(offersCache[i].count)
          end
        elseif categoryId == CATEGORY_OUTFIT then
          ...
          widget:setOn(not offersCache[i].unlocked)
          widget:setEnabled(not offersCache[i].unlocked)
          priceWidget:getChildById("value"):setText(offersCache[i].unlocked and "Unlocked" or comma_value(price))

          outfit:setOutfit(currentOutfit)
          outfit:show()
        elseif categoryId == CATEGORY_ADDONS then
          local outfit = imagePanel:getChildById("outfit")
          currentOutfit.type = offersCache[i].id
          currentOutfit.addons = offersCache[i].addon
...
function updateDescription(self)
  ...
  elseif type(self.data.id) == "number" then
    ...
    if categoryId == CATEGORY_ITEM then
      ...
      item:setItemCount(self.data.count)
    ...
    elseif categoryId == CATEGORY_OUTFIT then
      ...
      priceWidget:setText(comma_value(buyButton.price))
    elseif categoryId == CATEGORY_ADDONS then
      currentOutfit.type = self.data.id
      currentOutfit.addons = self.data.addon
...
function onOfferBuy(self)
  ...
  if selectedOffer.data.name == "Change Name" then
...
function buyConfirmed()
  ...
  if protocolGame then
    protocolGame:sendExtendedOpcode(GAME_STORE_CODE, json.encode({ action = "purchase", data = {
      ...
      category = selectedOffer.data.parent
    }}))
  end
...
function confirmChangeName()
  local protocolGame = g_game.getProtocolGame()
  if protocolGame then
    protocolGame:sendExtendedOpcode(GAME_STORE_CODE, json.encode({ action = "changeName", data = {
