...
local lastConfig = ""
config = Config.setup("cavebot_configs", configWidget, "json", function(name, enabled, data)
...
CaveBot.setCurrentProfile = function(name)
  if not g_resources.fileExists(context.configDir .. "/profiles/" .. g_game.getCharacterName() .. "/cavebot_configs/" .. name .. ".json") then
    return warn("there is no cavebot profile with that name!")
  end
  CaveBot.setOff()
  storage._configs[g_game.getCharacterName() .. "/cavebot_configs"].selected = name
  CaveBot.setOn()
end
